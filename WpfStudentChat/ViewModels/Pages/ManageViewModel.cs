    ...
    public async Task ImportUsers() // Id UserName Nickname Password
    {
        ...
        for (int i = 1; i <= sheet.LastRowNum; i++)
        {
            ...
            var user = new User()
            {
                ...
                UserName = row.GetCell(1)?.StringCellValue ?? string.Empty,
                Nickname = row.GetCell(2)?.StringCellValue ?? string.Empty,
            };

            var password = row.GetCell(3)?.StringCellValue;
        ...
        foreach (var (user,password) in users)
        {
            try
            {
                await _chatClientService.Client.AddUser(user, Convert.ToHexString(Encoding.UTF8.GetBytes(password)));
            }
            catch (Exception ex)
            {
